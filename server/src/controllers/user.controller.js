...
const getUserProfile = async (req, res, next) => {
  try {
    ...
    const watchlistCount = await WatchlistItem.countDocuments({ userId: user._id });
...
const followUser = async (req, res, next) => {
  try {
    ...
    if (isFollowing) {
      ...
      target.followers = target.followers.filter((id) => id.toString() !== currentUser._id.toString());
    } else {
...
const getUserWatchlist = async (req, res, next) => {
  try {
    ...
    const items = await WatchlistItem.find({ userId: user._id }).sort({ updatedAt: -1 });
    res.json(items);
  } catch (err) {
    next(err);
  }
